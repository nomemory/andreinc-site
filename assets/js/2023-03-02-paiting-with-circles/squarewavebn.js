const sqaureWaveBn = (s) => {
    ...
    s.setup = () => {
        ...
        canvas.parent('square-wave-f-bn-sketch');
    ...
    s.drawFsq = (buff, buffT, L, n) => {
        ...
        for (let x = 1; x < 2 * L; x += 0.5) {
            let cur = s.fsq(L, x) * s.sin(s.PI * n * x / L);
    ...
    s.coefficientBuf = (L, n) => {
        ...
        // Areas
        buff.push();
        buff.text("B" + n + "=", bC.x + 2 * step + 3 * r, bC.y - 10);
...
let sqaureWaveBnSketch =
    new p5(sqaureWaveBn, 'square-wave-f-bn-sketch');